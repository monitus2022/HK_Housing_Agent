...
class StorageManager:
    def __init__(self):
        self.s3_client = None
        self.service_type = settings.cloud_storage_service_type

        # Get configuration based on service type
        self.bucket_name = settings.cloud_storage_bucket_name
        self.region = settings.cloud_storage_bucket_region
        ...
        self.access_key_id = settings.cloud_storage_access_key_id
        self.secret_access_key = settings.cloud_storage_secret_access_key
    ...
    def _initialize_s3_client(self):
        ...
        try:
            ...
            backend_logger.info(f"{service_name} S3 client initialized successfully")
    ...
    def upload_files_from_data_folder(self):
        ...
        try:
            ...
            data_path = Path(settings.local_data_folder_path)

            if not data_path.exists():
                backend_logger.warning(f"Data folder {data_path} does not exist")
                return
            ...
            backend_logger.error(f"Upload failed: {e}")
            raise
    ...
    def _upload_file(self, local_file_path: str, s3_key: str):
        ...
        try:
            ...
            backend_logger.error(f"Failed to upload {local_file_path}: {e}")
            raise
    ...
    def _download_folder(self, folder_path: str):
        ...
        try:
            ...
            backend_logger.error(f"Download failed: {e}")
            raise
    ...
    def _download_file(self, s3_key: str, local_file_path: str):
        ...
        try:
            ...
            backend_logger.error(f"Failed to download {s3_key}: {e}")
            raise
    ...
    def download_all_from_datahub(self):
        ...
        """
        try:
            datahub_folder = settings.cloudflare_datahub_folder_name
            self._download_folder(datahub_folder)
        except Exception as e:
            backend_logger.error(f"Download from datahub failed: {e}")
            raise
